...
class School_Class(object):
    ...
    def __init__(self, db_obj, school_name='', class_name=''):
        ...
        if school_name == '' or class_name == '':
            ...
            self.Create()
        elif school_name in self.DB_obj.DB_Index['school_list'] and full_class_name in self.DB_obj.DB_Index[
            'class_list']:
    ...
    def Relation_Course(self):
        ...
        while True:
            ...
            if act.isdigit() and int(act) < len(course_list) and int(act) >= 0:
                self.Course_obj = Course(self.DB_obj, course_list_in_school[int(act)][1],
                                         course_list_in_school[int(act)][0])
                break
